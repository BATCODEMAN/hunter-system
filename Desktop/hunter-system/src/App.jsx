...
import AvatarSVG, { SKIN_TONES, HAIR_COLORS, EYE_COLORS, ARMOR_THEMES } from "./Avatar";
  ...
  async function handleSubmit() {
    ...
    if (isLogin) {
      ...
      if (error) { setError(error.message); }
        ...
        await supabase.from("profiles").insert({ id: data.user.id, username: username || email.split("@")[0], hunter_id: generateHunterId(), total_xp: 0, level: 1, strength: 10, endurance: 10, vitality: 10, agility: 10, mode: "home", streak: 0, longest_streak: 0, setup_complete: false });
        setMessage("Account created! Check your email to confirm, then log in."); setIsLogin(true);
      }
    }
    setLoading(false);
  }
  const inp = { width:"100%", background:"#0a0a0f", border:"1px solid #1e1e2e", borderRadius:8, padding:"10px 12px", color:"#e8e8f0", fontSize:14, boxSizing:"border-box" };
  return (
    <div style={{ background:"#0a0a0f", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"sans-serif", padding:20 }}>
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <div style={{ background:"#0f0f1a", border:"1px solid #1e1e2e", borderRadius:16, padding:24 }}>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{ width:"100%", background:"#534AB7", color:"#fff", border:"none", borderRadius:8, padding:"12px", fontSize:15, fontWeight:500, cursor:"pointer", opacity:loading?0.7:1 }}>{loading?"Loading...":isLogin?"Login":"Create Account"}</button>
        </div>
      </div>
    </div>
  );
...
function OnboardingScreen({ user, onComplete }) {
  ...
  return (
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", display:"flex", flexDirection:"column", alignItems:"center", padding:"30px 20px", maxWidth:420, margin:"0 auto" }}>
      ...
      {step >= 1 && <div style={{ width:80, marginBottom:16 }}><AvatarSVG rank="E" bodyType={answers.body_type||"average"} goal={answers.goal||"both"} gender={answers.gender||"male"} animated={false} /></div>}
...
function AvatarScreen({ user, profile, onClose, onUpdate }) {
  ...
  const [custom, setCustom] = useState({
    hair_style: profile?.hair_style || "short",
    hair_color: profile?.hair_color || "black",
    eye_color:  profile?.eye_color  || "purple",
  ...
  function ColorRow({ label, options, field, colorMap }) {
    return (
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:12, color:"#555", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.06em" }}>{label}</div>
    ...
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", maxWidth:420, margin:"0 auto", padding:"0 0 80px" }}>
        ...
        <div style={{ padding:"16px 20px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[["Strength",profile?.strength],["Endurance",profile?.endurance],["Vitality",profile?.vitality],["Agility",profile?.agility]].map(([name,val]) => (
              <div key={name} style={{ background:"#0f0f1a", border:"1px solid #1e1e2e", borderRadius:10, padding:"12px 14px" }}>
                <div style={{ fontSize:22, fontWeight:500 }}>{val||10}</div>
      ...
      {tab === "ranks" && (
        <div style={{ padding:"16px 20px" }}>
          ...
          {nextRankInfo && (
            <div style={{ background:"#0f0f1a", border:"1px solid #534AB7", borderRadius:12, padding:"14px", marginTop:8 }}>
  ...
  return (
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", maxWidth:420, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"12px 16px", display:"flex", flexDirection:"column", gap:10, minHeight:0, maxHeight:"calc(100vh - 180px)" }}>
        {messages.length === 0 && <div style={{ textAlign:"center", padding:"40px 0", color:"#444", fontSize:14 }}>No messages yet — be the first to speak!</div>}
        {messages.map(msg => {
...
function SocialScreen({ user, profile, onClose }) {
  ...
  const [friends, setFriends] = useState([]); const [leaderboard, setLeaderboard] = useState([]);
  useEffect(() => { loadData(); }, []);
  ...
  async function searchHunter() {
    if (!searchId.trim()) return; setSearching(true); setSearchResult(null);
  ...
  return (
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", maxWidth:420, margin:"0 auto", padding:"0 0 80px" }}>
      ...
      <div style={{ padding:"12px 16px" }}>
        ...
        {tab==="friends" && (
          <>
            {friends.length===0 && <div style={{ textAlign:"center", padding:"40px 0", color:"#444" }}>No friends yet!</div>}
            {friends.map((f,i) => { const r=getRank(f.level||1); return (
              <div key={i} style={{ background:"#0f0f1a", border:"1px solid #1e1e2e", borderRadius:10, padding:"10px 14px", marginBottom:8, display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:500 }}>{f.username}</div><div style={{ fontSize:11, color:"#555" }}>Lv{f.level} · 🔥{f.streak||0}</div></div>
    ...
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", maxWidth:420, margin:"0 auto", padding:"0 0 80px" }}>
      ...
      <div style={{ padding:"20px" }}>
        <div style={{ background:"#0f0f1a", border:"1px solid #1e1e2e", borderRadius:12, padding:"16px", marginBottom:14 }}>
          ...
          {permissionStatus==="denied" && <div style={{ background:"#2a0a0a", border:"1px solid #5a1a1a", borderRadius:8, padding:"10px 12px", fontSize:12, color:"#ff6b6b", marginTop:10 }}>Notifications are blocked in your browser settings.</div>}
        </div>
        {enabled && (
          <div style={{ background:"#0f0f1a", border:"1px solid #1e1e2e", borderRadius:12, padding:"16px", marginBottom:14 }}>
    ...
    <div style={{ background:"#0a0a0f", minHeight:"100vh", color:"#e8e8f0", fontFamily:"sans-serif", maxWidth:420, margin:"0 auto", padding:"0 0 80px" }}>
      ...
      {tab==="browse" && (
        <div style={{ padding:"14px 20px" }}>
          ...
          {filtered.length===0 && <div style={{ textAlign:"center", padding:"40px 0", color:"#444" }}>No quests yet!</div>}
          {filtered.map(q => {
...
export default function App() {
  ...
  if (screen==="avatar") return <AvatarScreen user={user} profile={profile} onClose={()=>setScreen("home")} onUpdate={updates=>setProfile(p=>({...p,...updates}))} />;
      ...
      <div style={{ background:"#0f0f1a", padding:"20px 20px 16px", borderBottom:"1px solid #1e1e2e" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <button onClick={()=>setScreen("avatar")} style={{ width:56, height:56, borderRadius:"50%", background:"#1a1035", border:"2px solid #534AB7", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", overflow:"hidden", padding:2, flexShrink:0 }}>
            <AvatarSVG rank={rank.rank} bodyType={profile?.body_type||"average"} goal={profile?.goal||"both"} gender={profile?.gender||"male"}
